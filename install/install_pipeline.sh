#!/bin/bash
# ============================================================
# SCRIPT D'INSTALLATION — Pipeline BottleNeck P10
# Tout est lancé depuis ~/P10/install/
# Structure :
#   ~/P10/Data/          → 3 fichiers Excel (inputs)
#   ~/P10/install/       → ce script, docker-compose, workflow
#   ~/P10/Output/        → livrables générés
#   ~/P10/Presentation/  → docs soutenance
# ============================================================

set -e

GREEN='\033[0;32m'
YELLOW='\033[1;33m'
RED='\033[0;31m'
BLUE='\033[0;34m'
NC='\033[0m'

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
P10_DIR="$(dirname "$SCRIPT_DIR")"
DATA_DIR="$P10_DIR/Data"
OUTPUT_DIR="$P10_DIR/Output"
INSTALL_DIR="$SCRIPT_DIR"

echo -e "${BLUE}============================================================${NC}"
echo -e "${BLUE}  INSTALLATION PIPELINE BOTTLENECK — P10 Data Engineer${NC}"
echo -e "${BLUE}============================================================${NC}"
echo ""
echo -e "  Projet:  $P10_DIR"
echo -e "  Data:    $DATA_DIR"
echo -e "  Output:  $OUTPUT_DIR"
echo ""

# -----------------------------------------------
# 0. Vérifier les fichiers Excel
# -----------------------------------------------
echo -e "${YELLOW}[0/4] Vérification des fichiers sources...${NC}"
MISSING=false
for f in "Fichier_erp.xlsx" "fichier_liaison.xlsx" "Fichier_web.xlsx"; do
    if [ -f "$DATA_DIR/$f" ]; then
        echo -e "${GREEN}  ✅ $f${NC}"
    else
        echo -e "${RED}  ❌ $f manquant dans $DATA_DIR/${NC}"
        MISSING=true
    fi
done
if $MISSING; then
    echo -e "${RED}Place les 3 fichiers Excel dans $DATA_DIR/ et relance.${NC}"
    exit 1
fi
mkdir -p "$OUTPUT_DIR"

# -----------------------------------------------
# 1. Vérifier Docker
# -----------------------------------------------
echo ""
echo -e "${YELLOW}[1/4] Vérification de Docker...${NC}"
if ! command -v docker &> /dev/null; then
    echo -e "${RED}Docker n'est pas installé.${NC}"
    echo "  sudo apt update && sudo apt install -y docker-ce docker-ce-cli containerd.io docker-compose-plugin"
    echo "  sudo usermod -aG docker \$USER && newgrp docker"
    exit 1
fi
if ! docker info &> /dev/null; then
    echo -e "${RED}Docker n'est pas accessible.${NC}"
    echo "  sudo usermod -aG docker \$USER && newgrp docker"
    exit 1
fi
echo -e "${GREEN}  ✅ Docker OK${NC}"
echo -e "${GREEN}  ✅ Docker Compose OK${NC}"

# -----------------------------------------------
# 2. Lancer Kestra + Postgres + MailHog
# -----------------------------------------------
echo ""
echo -e "${YELLOW}[2/4] Lancement de Kestra + MailHog...${NC}"

cd "$INSTALL_DIR"
docker compose up -d 2>&1 | tail -5

echo -e "${YELLOW}  Attente du démarrage (40s)...${NC}"
sleep 40

# Vérifier Kestra
KESTRA_OK=false
for i in {1..12}; do
    if curl -s http://localhost:8080 > /dev/null 2>&1; then
        KESTRA_OK=true
        break
    fi
    sleep 5
done
if $KESTRA_OK; then
    echo -e "${GREEN}  ✅ Kestra démarré: http://localhost:8080${NC}"
else
    echo -e "${YELLOW}  ⚠️  Kestra met du temps. Vérifie http://localhost:8080 dans 1-2 min.${NC}"
fi

# Vérifier MailHog
if curl -s http://localhost:8025 > /dev/null 2>&1; then
    echo -e "${GREEN}  ✅ MailHog démarré: http://localhost:8025${NC}"
else
    echo -e "${YELLOW}  ⚠️  MailHog pas encore prêt. Vérifie http://localhost:8025${NC}"
fi

# -----------------------------------------------
# 3. Trouver l'IP de MailHog et générer le workflow
# -----------------------------------------------
echo ""
echo -e "${YELLOW}[3/4] Génération du workflow...${NC}"

# Récupérer l'IP de MailHog sur le réseau bottleneck-network
MAILHOG_IP=$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' "$(docker compose ps -q mailhog)" 2>/dev/null)

if [ -z "$MAILHOG_IP" ]; then
    # Fallback : essayer par nom de container
    MAILHOG_IP=$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' mailhog 2>/dev/null || echo "mailhog")
fi

WORKFLOW_SRC="$INSTALL_DIR/workflow_bottleneck_smtp.yaml"
WORKFLOW_OUT="$INSTALL_DIR/workflow_ready.yaml"

if [ -f "$WORKFLOW_SRC" ]; then
    # Remplacer host.docker.internal par l'IP de MailHog
    sed "s|host.docker.internal|$MAILHOG_IP|g" "$WORKFLOW_SRC" > /tmp/wf_step1.yaml

    # Ajouter transportStrategy: SMTP après chaque ligne port: smtp_port
    awk '
    /port:.*smtp_port/ {
        print
        match($0, /^[[:space:]]*/)
        indent = substr($0, RSTART, RLENGTH)
        print indent "transportStrategy: SMTP"
        next
    }
    { print }
    ' /tmp/wf_step1.yaml > "$WORKFLOW_OUT"

    rm /tmp/wf_step1.yaml
    echo -e "${GREEN}  ✅ Workflow généré: workflow_ready.yaml${NC}"
    echo -e "${GREEN}     smtp_host = $MAILHOG_IP${NC}"
    echo -e "${GREEN}     transportStrategy = SMTP${NC}"
else
    echo -e "${RED}  ❌ $WORKFLOW_SRC non trouvé dans $INSTALL_DIR/${NC}"
    exit 1
fi

# -----------------------------------------------
# 4. Résumé
# -----------------------------------------------
echo ""
echo -e "${BLUE}============================================================${NC}"
echo -e "${BLUE}  INSTALLATION TERMINÉE ✅${NC}"
echo -e "${BLUE}============================================================${NC}"
echo ""
echo -e "  ${GREEN}Kestra:${NC}       http://localhost:8080"
echo -e "  ${GREEN}MailHog:${NC}      http://localhost:8025"
echo -e "  ${GREEN}SMTP:${NC}         $MAILHOG_IP:1025 (SMTP, pas SSL)"
echo -e "  ${GREEN}Réseau:${NC}       bottleneck-network"
echo ""
echo -e "  ${GREEN}Données:${NC}      $DATA_DIR/"
echo -e "  ${GREEN}Workflow:${NC}     $WORKFLOW_OUT"
echo -e "  ${GREEN}Outputs:${NC}      $OUTPUT_DIR/"
echo ""
echo -e "${BLUE}PROCHAINES ÉTAPES :${NC}"
echo ""
echo "  1. Ouvre Kestra        → http://localhost:8080"
echo "  2. Flows → + Create"
echo "  3. Colle le workflow :"
echo "     cat $INSTALL_DIR/workflow_ready.yaml"
echo "  4. Save → Execute → uploade les 3 Excel depuis $DATA_DIR/"
echo "  5. Vérifie l'email     → http://localhost:8025"
echo ""
